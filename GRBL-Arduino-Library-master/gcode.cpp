        ...
        if ( !( bit_false(axis_words,bit(gc.plane_axis_2)) ) || 
          ...
          if (r != 0) { // Arc Radius Mode
            /* 
              ...
              the center of the travel vector. A vector perpendicular to the travel vector [-y,x](VECTOR CHI PHUONG) is scaled to the 
              length of h [-y/d*h, x/d*h](TI LE VECTOR CHI PHUONG THEO HE SO d va h) and added to the center of the travel vector [x/2,y/2] to form the new point 
              [i,j] at [x/2-y/d*h, y/2+x/d*h] which will be the center of our arc.
              
              d^2 == x^2 + y^2  // Do dai vector
              h^2 == r^2 - (d/2)^2
              i == x/2 - y/d*h   ///  y/d*h LA HE SO THAY DOI THEO BAN KINH R 
              j == y/2 + x/d*h	///  x/d*h LA HE SO THAY DOI THEO BAN KINH R 
